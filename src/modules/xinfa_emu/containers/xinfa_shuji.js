...
class XinfaShuji extends Component {
        ...
        return(
          <div key={skill.name} styleName='xinfa-skill'>
            ...
            <Dropdown bsSize='xsmall' id={`${skill.name}-dropdown`} dropup>
              <Dropdown.Toggle bsStyle='primary' noCaret>LV.{this.props.skillLevelsData[skillName] || 0}</Dropdown.Toggle>
                  ...
                  skill.levels.map((level, i) => (
                    ...
                    >
                      ...
                      <div styleName='skill-dropdown-item-des'>
                        ...
                        <div>필요 수양：{level.xiuwei}</div>
                        <div>누적 수양：{level.xiuweiSum || 0}</div>
                        { level.shayi && <div>소모：{level.shayi}</div>}
                      </div>
                    </MenuItem>
                  )).reverse()
                }
  ...
  fastBrkthruShuji(level) {
    if(window.confirm('선택한 단계까지 일괄 충혈하시겠습니까?')) {
  ...
  renderQianxiu() {
    let maxLevel = 25;
    if(this.props.xinfaData.name.startsWith('연무'))  // todo 更优雅的方式
      maxLevel = 5;
    return [
      {name: '힘', id: 'ld'},
      {name: '기력', id: 'qj'},
              ...
              <Dropdown.Menu styleName='qianxiu-dropdown-menu'>
                {
                  range(maxLevel + 1).map((level) => (
                    ...
                    >
                      ...
                      현재 정진도 {String('00000' + qianxiuPointList[level][1]).slice(-5)}
                    </MenuItem>
                  )).reverse()
                }
              </Dropdown.Menu>
  ...
  render() {
    ...
    return (
    <WuxiaPanel title='심법단계'>
        {
          this.props.xinfaData.name  &&
            <div>
              <div styleName='xinfa-reinforce'>
                <span styleName='reinforce-title'>상생</span>
          ...
          { this.props.xinfaData.name &&
            <div>
              <div styleName='xinfa-skills'>
                <span styleName='xinfa-skills-title'>스킬</span>
        ...
        { this.props.xinfaData.name &&
          <div>
            ...
            <div styleName='level-button-group'>
              <ButtonGroup>
                <Button disabled styleName='view-btn-regular'>살펴보기</Button>
                {this.renderViewButtons('view')}
                <Button disabled styleName='view-btn-regular'>완</Button>
              </ButtonGroup>
            </div>
            <div styleName='level-button-group'>
              <ButtonGroup>
                <Button styleName='view-btn-regular-with-hover'>
                  <span className='normal'>일괄채우기</span>
                  <span className='hover'>일괄채우기</span>
                </Button>
